...
public enum MoxSource : byte
{
    ...
    QrmText = 9,
    /// <summary>The in-core FT8/FT4 slot keyer (<c>Ft8KeyerService</c>) —
    /// the fused-build successor to the reserved <see cref="Ft8"/> era. Keys
    /// only at a slot boundary while the operator has ARMED the keyer, releases
    /// only the MOX edge it claimed (watchdog + HALT force-release through the
    /// same token), and <see cref="UI"/> remains the master override. Never
