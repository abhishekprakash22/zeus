...
//
// FT8/FT4 constants — ported from ft8_lib's ft8/constants.{h,c} (Karlis Goba,
// MIT; see LICENSE.ft8_lib). The tables are copied value for value; the LDPC
// ones come from WSJT-X (ldpc_174_91_c_*.f90).

namespace Zeus.Server.Hosting.Digital.Ft8;

internal static class FtxConstants
{
    public const float Ft8SymbolPeriod = 0.160f;
    public const float Ft8SlotTime = 15.0f;
    public const float Ft4SymbolPeriod = 0.048f;
    public const float Ft4SlotTime = 7.5f;

    // FT8: S7 D29 S7 D29 S7.
    public const int Ft8Nd = 58;
    public const int Ft8Nn = 79;
    public const int Ft8LengthSync = 7;
    public const int Ft8NumSync = 3;
    public const int Ft8SyncOffset = 36;

    // FT4: R Sa D29 Sb D29 Sc D29 Sd R.
    public const int Ft4Nd = 87;
    public const int Ft4Nr = 2;
    public const int Ft4Nn = 105;
    public const int Ft4LengthSync = 4;
    public const int Ft4NumSync = 4;
    public const int Ft4SyncOffset = 33;

    // LDPC (174,91).
    public const int LdpcN = 174;
    public const int LdpcK = 91;
    public const int LdpcM = 83;
    public const int LdpcNBytes = (LdpcN + 7) / 8;
    public const int LdpcKBytes = (LdpcK + 7) / 8;

    // CRC-14, polynomial without the leading 1.
    public const ushort CrcPolynomial = 0x2757;
    public const int CrcWidth = 14;

    public static ReadOnlySpan<byte> Ft8Costas => [3, 1, 4, 0, 6, 5, 2];

    /// <summary>Four 4-tone Costas groups, flattened [group * 4 + i].</summary>
    public static ReadOnlySpan<byte> Ft4Costas => [0, 1, 3, 2, 1, 0, 2, 3, 2, 3, 1, 0, 3, 2, 0, 1];

    /// <summary>Gray code: bits → channel tone.</summary>
    public static ReadOnlySpan<byte> Ft8Gray => [0, 1, 3, 2, 5, 6, 4, 7];
    public static ReadOnlySpan<byte> Ft4Gray => [0, 1, 3, 2];

    /// <summary>FT4 whitening, XORed onto the payload before CRC and LDPC.</summary>
    public static ReadOnlySpan<byte> Ft4Xor => [0x4A, 0x5E, 0x89, 0xB4, 0xB0, 0x8A, 0x79, 0x55, 0xBE, 0x28];

    /// <summary>Parity generator, bit-packed MSB first: row i at [i * LdpcKBytes].</summary>
    public static ReadOnlySpan<byte> LdpcGenerator =>
    [
    ...
    /// <summary>Parity checks: row i at [i * 7], 1-origin codeword indices, 0 = unused.</summary>
    public static ReadOnlySpan<byte> LdpcNm =>
    [
    ...
    /// <summary>For each codeword bit (row i at [i * 3]), its three parity checks, 1-origin.</summary>
    public static ReadOnlySpan<byte> LdpcMn =>
    [
        16, 45, 73,
        25, 51, 62,
        33, 58, 78,
    ...
    /// <summary>Number of used entries in each row of LdpcNm.</summary>
    public static ReadOnlySpan<byte> LdpcNumRows =>
    [
