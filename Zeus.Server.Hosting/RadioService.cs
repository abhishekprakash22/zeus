...
public sealed class RadioService : IDisposable
{
    ...
    // Persisted-filter sanity envelope. 50 Hz mirrors the frontend's
    // MIN_PASSBAND_HZ; 24 kHz is the audio Nyquist at the lowest sample rate
    // (48 k), so no legitimate passband edge can sit beyond it. Values are
    // deliberately loose — the net exists to catch corrupt/legacy rows
    // (crossed edges, zero width, garbage magnitudes), not to police wide
    // operator choices.
    private const int MinFilterWidthHz = 50;
    ...
    public void SetMultiEncoderFunction(string name)
    {
        if (Snapshot().MultiEncoderFunction == name) return;
