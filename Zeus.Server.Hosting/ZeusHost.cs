...
public static class ZeusHost
{
    ...
    public static WebApplication Build(string[] args, ZeusHostOptions options)
    {
        ...
        // registry that is now 404. Shipping them here means `dotnet publish`
        // produces a complete radio: nothing to install, nothing to hand-copy.
        // Routes are mapped under the SAME prefix the frontend already targets
        // (/api/plugins/org.openhpsdr.digital/...), so zeus-web is untouched.
        builder.Services.AddSingleton<Zeus.Server.Hosting.Digital.DigitalService>();
        builder.Services.AddHostedService(sp =>
        ...
        // WSPR in core — 120 s slot decoder (a C# port of K9AN's wsprd,
        // Digital/Wspr) + autonomous beacon keyer. Completes the Digital
        ...
        // Core digital-mode routes. Mapped BEFORE PluginEndpoints.MapAll so that
        // if a real org.openhpsdr.digital plugin is ever installed alongside, the
        ...
        app.MapPost("/api/pa-cal/measure", (PaCalStartRequest req, PaCalibrationService cal) =>
        {
            var result = cal.Measure(req.Confirm, out var refusal);
            return refusal is null ? Results.Ok(result) : Results.BadRequest(new { error = refusal });
        });
