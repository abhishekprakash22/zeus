	...
	int type, int state, double tdelayup, double tslewup, double tdelaydown, double tslewdown, int bfo)
{
	...
	// Zeus: _MM_SET_FLUSH_ZERO_MODE is an x86 SSE-only perf hint (denormals
	// → zero in the MXCSR). It does not exist on ARM64 toolchains (MSVC
	// clang-cl, gcc, clang) and <xmmintrin.h> isn't available there either,
	// so the call is gated out for Windows-on-ARM and aarch64 in addition to
	// the linux / Apple gates. ARM64 has an analogous FZ bit in FPCR; WDSP
	// does not depend on FTZ for correctness so FPCR is left at the OS
	// default rather than introducing an architecture-specific intrinsic.
